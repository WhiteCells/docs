...
当有多个互斥量保护同一个类的独立实例时，一个操作对同一个类的两个不同实例进行数据的交换操作时，为了保证数据交换的正确性，就要避免并发修改数据，并确保每个实例上的互斥量都能锁住自己要保护的区域，如果选择一个固定的顺序，如实例提供的第一个互斥量作为第一个参数，实例提供的第二个互斥量作为第二个参数，可能会出现：在参数交换后，两个线程试图在相同的两个实例间进行数据交换时，程序死锁。
